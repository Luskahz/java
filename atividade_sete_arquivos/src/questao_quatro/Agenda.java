...
public Contato lerContatoPorNome(String nomeBuscado, String nomeArquivo) {
    try (BufferedReader reader = new BufferedReader(new FileReader(nomeArquivo))) {
        ...
        while ((linha = reader.readLine()) != null) {
            ...
            if (dados.length < 4) continue; // evita erro em linhas incompletas
            ...
            if (nome.equalsIgnoreCase(nomeBuscado.trim())) {
                ...
                if (tipo.equalsIgnoreCase("PessoaFisica")) {
                    ...
                    String nascimento = dados[5].trim();
                    String estadoCivil = dados[6].trim();
                    return new PessoaFisica(nome, endereco, email, cpf, nascimento, estadoCivil);
                } else if (tipo.equalsIgnoreCase("PessoaJuridica")) {
                    String cnpj = dados[4].trim();
                    String inscricaoEstadual = dados[5].trim();
                    String razaoSocial = dados[6].trim();
                    return new PessoaJuridica(nome, endereco, email, cnpj, inscricaoEstadual, razaoSocial);
                }
            }
        }
